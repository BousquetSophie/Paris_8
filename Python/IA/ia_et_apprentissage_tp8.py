# -*- coding: utf-8 -*-
"""IA_et_apprentissage_TP8.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1s1Ir9JeDivMOwVgro-i9YDTMdcnrlYMB
"""
...
class HierarchicalAttention(torch.nn.Module):
  ...
  def forward(self, inputs):
    """
    Une fonction qui prend un tensor en entrée et renvoie sa sortie comme la sortie d'un module d'attention hierarchique
    :param inputs: Un tenseur 3D de float32 de shape [batch_size, sequence_length, in_features]
    :return:
    """

    # On fabrique notre sequence de vecteur de queries, de shape [batch_size, sequence_length, out_features]
    queries = self.query_layer(inputs)

    # On fabrique notre sequence de vecteur de key, de shape [batch_size, sequence_length, out_features]
    keys = self.key_layer(inputs)

    # On fabruique notre sequence de vecteur de values, de shape [batch_size, sequence_length, out_features]
    values = self.value_layer(inputs)

    pre_ponderation = queries * keys

    # On somme pour obtenir le produit scalaire et avoir un tenseur de shape [batch_size, sequence_length]
    pre_ponderation = pre_ponderation.sum(-1)
    ponderation = torch.nn.Softmax(dim=-1)(pre_ponderation)

    # On multiplie nos entrées et nos pondérations pour avoir un tenseur de shape [batch_size, sequence_length, out_features]
    output_ponderees = ponderation.unsqueeze(-1) * values

    outputs = output_ponderees.sum(1)
...
class AttentionModule(torch.nn.Module):
  ...
  def forward(self, inputs):
    ...
    #Tenseur de shape [batcsh_size, sequence_lengeth, sequence_lengeth],  les preponderations pour chacun des tokens
    pre_ponderations = torch.matmul(queries, keys.transpose(1,2))

    ponderations = torch.nn.Softmax(dim=-1)(pre_ponderations)

    outputs = torch.matmul(ponderations.transpose(1,2), values)
