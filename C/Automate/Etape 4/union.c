...
// Union des 2 Automate :
	...
	for(int i = 0; i < nb_occ1; i++)
	{
		...
		{
			if(strcmp(tab_occ1[i], tab_occ2[j]) == 0)
			{
				for(int k = 0; k <= 10; k++)
				{
					if(strcmp(tab_occ1[i], union_auto[k]) == 0)
						verif_doublon++;
						occurence[k]++;
				}
				if(verif_doublon == 0)
				{
					strcat(union_auto[e],tab_occ1[i]);
					nb_motif ++;
					occurence[e] = 1;
					e++;
				}
	...
	printf("Union des motifs trouvés sans doublons\n");
